...
(function($) {
  ...
  $(function() {
    ...
    function addActiveClass(element) {
	    var existing = element.attr('href').split("/").slice(-1)[0].replace(/^\/|\/$|#/g, '');
	    
	    if(element.attr('href').split("/").length === 3){
			existing =  element.attr('href').split("/").slice(-2)[0].replace(/^\/|\/$|#/g, '') + element.attr('href').split("/").slice(-1)[0].replace(/^\/|\/$|#/g, '');
		}
		
        if (current === "") {
          //for root url
          if (element.attr('id').indexOf("dashboard-custom") !== -1) {
      ...
      var addons = location.hash.replace("#", "");
      var current = location.pathname.split("/").slice(-1)[0].replace(/^\/|\/$|#/g, '') + addons;
      
    if(location.pathname.split("/").length === 3){
		current =  location.pathname.split("/").slice(-2)[0].replace(/^\/|\/$|#/g, '') + location.pathname.split("/").slice(-1)[0].replace(/^\/|\/$|#/g, '') + addons;
	}
