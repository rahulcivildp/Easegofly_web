...
import com.easygofly.api.setting.GeneralSettingBag;
...
public class OrderPDFExporter extends AbstractOrderExporter {
	...
	public Document export(Order order, HttpServletResponse response) throws Exception {
		super.setResponseHeader(response, "application/pdf", ".pdf");

		// Set headers
		response.setContentType("application/pdf");
		String headerKey = "Content-Disposition";
		String headerValue = "attachment; filename=ticket_" + order.getId() + ".pdf";
		response.setHeader(headerKey, headerValue);

		ProductDetail flight = order.getProductDetails().get(0);
//		ProductDetail flight = productDetailCrudRepo.findById(227).get();
		List<TravellerDetail> travellerDetails = order.getTravellerDetails();
		City cityOne = cityRepo.getCityByCode(flight.getCityOne());
		City cityTwo = cityRepo.getCityByCode(flight.getCityTwo());
		...
		if (brand == null) {
			brand = new Brand(flight.getBrand());
		...
		addressAndBarCode(cb, new PdfPTable(3), document, order, faviconLink, flight);
		flightItinary(new PdfPTable(3), document, order, cityOne, cityTwo, flight);
		flightDetailsHeader(new PdfPTable(1), document, order, cityOne, cityTwo, brand, flight);
		flightDetails(new PdfPTable(5), document, order, cityOne, cityTwo, brand, flight);
		travelerDetailsHeader(new PdfPTable(2), document, order, flight);
		...
		travellerInfo(cb, new PdfPTable(3), document, order, flight);
	...
	private void addressAndBarCode(PdfContentByte cb, PdfPTable table, Document document, Order order,
			String faviconLink, ProductDetail flight) throws Exception {
	...
	private void flightDetailsHeader(PdfPTable table, Document document, Order order, City cityOne, City cityTwo,
			Brand brand, ProductDetail flight) throws BadElementException, IOException {
		table.setWidthPercentage(100f);
		table.setSpacingBefore(10);
	...
	private void flightDetails(PdfPTable table, Document document, Order order, City cityOne, City cityTwo, Brand brand,
			ProductDetail flight) throws BadElementException, IOException {
		table.setWidthPercentage(100f);
		...
		Chunk duration1 = (durationMinutes < 10)
		...
		Chunk duration2 = (stopsInt != 0) ? new Chunk("STOPS: " + stopsInt.toString() + "\n", fontRed)
				: new Chunk("NON STOP\n", fontRed);

		String journeyType = (flight.getJourneyClass().equals("1")) ? "Economy"
				: (flight.getJourneyClass().equals("2")) ? "PremiumEconomy"
						: (flight.getJourneyClass().equals("3")) ? "Business"
								: (flight.getJourneyClass().equals("4")) ? "PremiumBusiness"
										: (flight.getJourneyClass().equals("5")) ? "FirstClass" : "All";

		Chunk duration3 = new Chunk(journeyType, fontFancy);
	...
	private void travelerDetailsHeader(PdfPTable table, Document document, Order order, ProductDetail flight)
		...
		Chunk baggageChk = new Chunk("Baggage info Check-In: " + flight.getBaggage() + ", Cabin: "
				+ flight.getCabinBaggage() + " | LowFare");
	...
	private void travelerTableHeader(PdfPTable table, Document document, Order order)
			throws BadElementException, IOException {
		table.setWidthPercentage(100f);
	...
	private void travelerTableBody(PdfPTable table, Document document, Order order, TravellerDetail travellerDetail,
			ProductDetail productDetail) throws BadElementException, IOException {
		table.setWidthPercentage(100f);
		table.setSpacingBefore(2);
		table.setWidths(new float[] { 3.5f, 2.2f, 2.2f, 2.2f, 2.2f, 2.2f });
		...
		border = new Rectangle(0f, 0f);
		travellerTableHeaderPart2(border, cell, color, color2);
		String price = (travellerDetail.getPaxType().equals("3")) ?
			 Double.toString(productDetail.getPriceINF()) : Double.toString(productDetail.getPriceADT());
	...
	private void totalFareTable(PdfPTable table, Document document, Order order)
			throws BadElementException, IOException {
		table.setWidthPercentage(100f);
		table.setSpacingBefore(2);
		...
		grossPhrs.add(grossChk);
		grossPhrs.add(grossChk1);
		...
		pricePhrs.add(priceChk);
		pricePhrs.add(priceChk1);
	...
	private void extraDetailsAfterPrice(PdfPTable table, Document document, Order order, String siteLogo)
			throws BadElementException, IOException {
		table.setWidthPercentage(100f);
		...
		extraPhrs.add(extraChk);
		extraPhrs.add(extraChk1);
	...
	private void travellerInfo(PdfContentByte cb, PdfPTable table, Document document, Order order, ProductDetail flight)
			throws BadElementException, IOException {
		table.setWidthPercentage(100f);
		...
		infoPhrs.add(infoChk);
		infoPhrs.add(infoChk1);
		infoPhrs.add(infoChk2);
	...
	private void additionalInfo(PdfPTable table, Document document) throws BadElementException, IOException {
		...
		Chunk normal1 = new Chunk(
				"2. Check-in counter closes strictly 45-mins prior departure time. Please Check-in atleast 2.5 Hrs prior departure"
						+ "time.\r\n",
				fontSmall);
		Chunk normal2 = new Chunk(
				"3. Delay & Cancellation of Flights are out of our or airline’s control. Please get in touch with Airline Staffs for"
		...
		Chunk normal4 = new Chunk(
				"5. Fare once booked, cannot be discounted further even if system fare reduces & is an agreement between buyerseller.\r\n",
				fontSmall);
	...
	private void footerDialog(PdfPTable table, Document document) throws BadElementException, IOException {
		...
		Chunk bold = new Chunk(
				"Naws Tour & Travels is not liable for any Discrepancy / Deficiency in service by the Airline or Service Providers.. Any discrepancy regarding this ticket,"
	...
	private void footerInfo(PdfPTable table, Document document) throws BadElementException, IOException {
		...
		Chunk text = new Chunk(
				"+ BAGGAGE DISCOUNT MAY APPLY BASSED ON FREQUEST FLYER STATUS/ONLINE CHECKIN/FORM OF PAYMENT/MILITARY/ETC.",
				font);
		Chunk text1 = new Chunk("Email: support@easegofly.com \s\sMobile: +91-8348000139 \s\sWhatsapp: +91-9732504064",
				font);
